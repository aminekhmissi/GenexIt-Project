...
module.exports = {
  async createReservation(req, res) {
    try {
      ...
      if (
        ...
        moment(newReservation.checkIn).isSameOrAfter(lodge.dateDebut) &&
        moment(newReservation.checkOut).isSameOrBefore(lodge.datefin) &&
        ...
        if (ExistedReservation === false) {
          ...
          msg =
            "can't reserve include other reservation : other reservation exists ";
        }
      } else {
  ...
  async confirmReservation(req, res) {
    try {
      ...
      const lodge = await Lodge.findById(reservation.lodge);
      const customer = await Customer.findById(reservation.customer);
      const owner = await Owner.findById(lodge.owner);
      await transport.sendMail(
        {
          to: owner.email,
          subject: "your lodge is reserved !!",
          text: "welcome " + owner.fullname,
          html: `
       <!DOCTYPE html>
       <html>
         <head>
               ...
               <![endif]-->
                 ...
                 >
                   ...
                   <tr>
                     ...
                     >
                       <p style="margin: 0">
                        we inform you :your lodge ${
                          lodge.title
                        } reserved started at ${moment(
            reservation.checkIn
          ).format("dddd, MMMM Do YYYY")}, ended at ${moment(
            reservation.checkOut
          ).format("dddd, MMMM Do YYYY")} by the customer :${customer.fullname}
